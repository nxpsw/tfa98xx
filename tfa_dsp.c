...
#define TFA_RAMPDOWN_BEFORE_MUTE
...
#if defined(TFA_USE_DEVICE_SPECIFIC_CONTROL)
	if (((set_count % devcount) == 0 && active_handle == -1)
		|| (tfa->dev_idx == active_handle)) /* at initial device only */
#else
	if ((set_count % devcount) == 0) /* at initial device only */
#endif
	{
		need_cal = 0;
...
#if defined(TFA_USE_DEVICE_SPECIFIC_CONTROL)
		if (((set_count % devcount) == 0 && active_handle == -1)
			|| (tfa->dev_idx == active_handle)) /* at last device only */
#else
		if ((set_count % devcount) == 0) /* at last device only */
#endif
		{
			set_count = 0;
			dsp_cal_value[0] = dsp_cal_value[1] = -1;
...
static enum tfa_error tfa_gain_rampdown(struct tfa_device *tfa, int count)
{
	enum tfa98xx_error err = TFA98XX_ERROR_OK;
	int cur_ampgain;
	int i;
...
static enum tfa_error tfa_gain_restore(struct tfa_device *tfa)
{
	enum tfa98xx_error err = TFA98XX_ERROR_OK;
	int cur_ampgain;
	...
	cur_ampgain = tfa_get_bf(tfa, TFA2_BF_AMPGAIN);
	if (cur_ampgain > 0) {
		pr_debug("%s: ampgain is non-zero (%d), ampgain = %d\n",
	...
	int next_profile, int vstep)
{
	...
	if (active_handle != -1)
		pr_info("%s: active handle [%s]", __func__,
			tfa_cont_device_name(tfa->cnt, active_handle));
#endif

	if (active_handle != -1) {
		if (tfa->dev_idx != active_handle) {
...
#if defined(TFA_RAMPDOWN_BEFORE_MUTE)
		/* ramp down gain at muting only */
		if ((state & 0x0f) == 0
			&& is_calibration == 0)
			tfa_gain_rampdown(tfa, 5);
#endif
...
#define LOWER_LIMIT_CAL_N1C 0
#define UPPER_LIMIT_CAL_N1C 32000
#define LOWER_LIMIT_CAL_P_N1C 0
#define UPPER_LIMIT_CAL_P_N1C 32000
#define LOWER_LIMIT_CAL_S_N1C 0
#define UPPER_LIMIT_CAL_S_N1C 32000
static enum tfa98xx_error
tfa_calibration_range_check(struct tfa_device *tfa,
	unsigned int channel, int mohm)
{
	...
	lower_limit_cal = TFA_CAL_RANGE(spkr_count, channel, LOWER, N1C);
	upper_limit_cal = TFA_CAL_RANGE(spkr_count, channel, UPPER, N1C);
